...
import { IconBrain, IconPhone, IconArrowRight, IconAmbulance } from '../components/icons.jsx';

export default function Landing() {
  return (
    <div className="min-h-full bg-gradient-to-br from-primary via-primary-700 to-accent text-white flex flex-col">
      <div className="max-w-4xl w-full mx-auto px-5 lg:px-8 pt-8 pb-4 flex items-center gap-2.5">
      ...
      <main className="flex-1 flex items-center">
        <div className="max-w-3xl w-full mx-auto px-5 lg:px-8 py-10 lg:py-16 text-center">
          ...
          <div className="mt-9 lg:mt-12 mx-auto max-w-xl space-y-4">
            <Link
              to="/triage"
              className="group rounded-2xl bg-danger text-white p-6 lg:p-7 shadow-card hover:shadow-2xl transition flex items-center gap-5 text-left"
            >
            ...
            {/* Accesso operatori: card grande alla pari della CTA cittadini
              * (richiesta cliente rev. 2 — prima era un link piccolo in alto). */}
            <Link
              to="/login"
              className="group rounded-2xl bg-white text-primary p-6 lg:p-7 shadow-card hover:shadow-2xl transition flex items-center gap-5 text-left"
            >
              <div className="p-3 rounded-xl bg-accent-50 text-accent shrink-0">
                <IconAmbulance className="w-8 h-8" />
              </div>
              <div className="flex-1">
                <div className="text-xs uppercase tracking-widest text-primary-700">Personale sanitario</div>
                <div className="text-2xl lg:text-3xl font-extrabold mt-1 text-primary-900">Accesso operatori</div>
                <div className="text-primary-700 mt-1 text-sm">
                  118 · Medico HUB · Centrale Operativa
                </div>
              </div>
              <IconArrowRight className="w-6 h-6 text-primary-100 group-hover:text-accent shrink-0" />
            </Link>
          </div>
